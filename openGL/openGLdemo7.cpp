...
void keyPressed (unsigned char key, int x, int y) {  
    ...
    static double size = .4;  //shapePerson's size...we may want this to change
    //remember, static means the value of the variable is preserved between function calls

    //double i = startPos; //for loop increment
    ...
    if(key == 'f'){
        ...
        cout << "Grow!!" << endl;
        glClear(GL_COLOR_BUFFER_BIT);        
        drawShapePerson(startPos, size);  //use last known location
        size += .05;    
        glFlush(); 
    }
